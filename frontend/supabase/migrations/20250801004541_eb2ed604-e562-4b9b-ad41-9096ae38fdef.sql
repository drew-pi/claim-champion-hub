...
INSERT INTO public.profiles (user_id, email, first_name, last_name, phone, role) VALUES
  (gen_random_uuid(), 'john.patient@email.com', 'John', 'Doe', '555-0101', 'patient'),
  (gen_random_uuid(), 'jane.advocate@email.com', 'Jane', 'Smith', '555-0102', 'advocate'),
  (gen_random_uuid(), 'admin@healthadvocate.com', 'Admin', 'User', '555-0103', 'admin');

-- Insert sample claims
INSERT INTO public.claims (patient_id, policy_number, employer_name, healthcare_provider_name, claim_description, claim_amount, claim_date, status) VALUES
  ((SELECT id FROM public.profiles WHERE email = 'john.patient@email.com'), 'POL-12345', 'Tech Corp Inc.', 'City Medical Center', 'Emergency room visit for chest pain', 2500.00, '2024-01-15', 'under_review'),
  ((SELECT id FROM public.profiles WHERE email = 'john.patient@email.com'), 'POL-12345', 'Tech Corp Inc.', 'Orthopedic Specialists', 'MRI scan for knee injury', 1200.00, '2024-01-20', 'approved');